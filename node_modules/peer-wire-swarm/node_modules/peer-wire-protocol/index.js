...
var Wire = function(opts) {
	...
	this.on('finish', function() {
		...
		//modif
		//console.log('wire finished '+self.peerAddress,true);
		self.emit('end');
	});

	var ontimeout = function() {
		//console.log('request timeout for '+self.peerAddress);
	...
	var onmessage = function(buffer) {
		if (!this.freerider) {
			try {
				//console.log('Got message from '+this.peerAddress+' '+buffer.toString('hex'));
			} catch(ee) {};
		};
		self._parse(4, onmessagelength);
		switch (buffer[0]) {
...
Wire.prototype._oninterested = function() {
	if (!this.freerider) {
		//console.log('Peer '+this.peerAddress+' is interested ');
	};
...
Wire.prototype._onuninterested = function() {
	if (!this.freerider) {
		//console.log('Peer '+this.peerAddress+' is not interested ');
	};
	this.peerInterested = false;
...
Wire.prototype._onchoke = function() {
	if (!this.freerider) {
		//console.log('Peer '+this.peerAddress+' sent choke ');
	};
	this.peerChoking = true;
	this.emit('choke');
...
Wire.prototype._onunchoke = function() {
	if (!this.freerider) {
		//console.log('Peer '+this.peerAddress+' sent unchoke ');
	};
...
Wire.prototype._onbitfield = function(buffer) {
	if (!this.freerider) {
		//console.log('Peer '+this.peerAddress+' sent bitfield ');
	};
...
Wire.prototype._onhave = function(i) {
	if (!this.freerider) {
		//console.log('Peer '+this.peerAddress+' has piece '+i);
	};
...
Wire.prototype._onrequest = function(i, offset, length) {
	if (!this.freerider) {
		//console.log(' piece '+i+' offset '+offset+' length '+length+' requested by '+this.peerAddress);
	};
...
Wire.prototype._onpiece = function(i, offset, buffer) {
	if (!this.freerider) {
		//console.log('Peer '+this.peerAddress+' sent piece '+i);
	};
...
Wire.prototype._onport = function(port) {
	if (!this.freerider) {
		//console.log('Peer '+this.peerAddress+' sent port '+port);
	};
	this.emit('port', port);
};
...
Wire.prototype._onextended = function(id, ext) {
	if (!this.freerider) {
		//console.log('Peer '+this.peerAddress+' sent extended '+id);
	};
...
Wire.prototype._message = function(id, numbers, data) {
	...
	//console.log('sending message id '+id+' to '+this.peerAddress);
	this._push(buffer);
