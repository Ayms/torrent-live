...
var onwire = function(swarm, connection, onhandshake) {
	...
	var destroy = function(txt) {
		//console.log(txt+' pws timeout for '+wire.peerAddress);
		//modif
		wire.__timeout__=true;
		connection.destroy();
	};
	...
	wire.once('handshake', function(infoHash, peerId) {
		...
		//modif
		swarm.emit('handshake',wire,peerId);
	...
	connection.on('error', function(err) {
		...
		try {
			swarm.emit('error',wire,err);
		} catch(ee) {};
		connection.destroy();
	});

	connection.on('close', function() {
		//console.log('--------- connection close '+connection.remoteAddress+':'+connection.remotePort,true);
...
Swarm.prototype._drain = function() {
	...
	wire.on('end',retry_or_remove);
	
	wire.on('close',retry_or_remove);
	
	//add wire.on('close',idem) #10
