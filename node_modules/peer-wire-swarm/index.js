...
var onwire = function(swarm, connection, onhandshake) {
	...
	wire.once('handshake', function(infoHash, peerId) {
		//console.log('--------- handshake '+connection.remoteAddress+':'+connection.remotePort);
		clearTimeout(handshakeTimeout);
		onhandshake(infoHash, peerId);
	});

	connection.on('end', function() {
		//console.log('--------- end '+connection.remoteAddress+':'+connection.remotePort);
		connection.destroy();
	});

	connection.on('error', function() {
		//console.log('--------- error '+connection.remoteAddress+':'+connection.remotePort);
		connection.destroy();
	});
...
Swarm.prototype._drain = function() {
	...
	//console.log('--------------- calling onwire '+addr);
	
	var wire = onwire(this, connection, function(infoHash) {
		console.log('--------------------- try onwire - remote '+connection.remoteAddress+':'+connection.remotePort+' - from local port '+connection.address().port+' infohash '+infoHash.toString('hex')+' self infohash '+self.infoHash.toString('hex'));
		if (infoHash.toString('hex') !== self.infoHash.toString('hex')) return connection.destroy();
		console.log('------------- onwire - connected to '+connection.remoteAddress+':'+connection.remotePort+' - from local port '+connection.address().port);
		peer.reconnect = true;
		peer.retries = 0;
