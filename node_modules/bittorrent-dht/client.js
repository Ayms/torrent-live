...
function DHT (opts) {
  ...
  if (opts.bootstrap === false) {
	  ...
	  //console.log('no bootstrap emit ready',true);
...
DHT.prototype._onListening = function () {
  ...
  self.listening = true
  //modif
  self.emit('listening', self.socket.address().port)
}
...
DHT.prototype.destroy = function (cb) {
  ...
  self.freerider=null
  self.secrets=null
  self.closest_from_infohash=null
 ...
 */
 //modif
DHT.prototype._addPeer = function (addr, infoHash, _addr, id, version) {
  var self = this
  ...
  if (!exists) {
    ...
    self.emit('peer', addr, infoHash, _addr, id, version)
...
DHT.prototype.lookup = function (id, opts, cb) {
  ...
  function query (addr,nid) {
    ...
    if (opts.findNode) {
      ...
      self._sendGetPeers(addr, id, onResponse, nid)
    }
  }
  ...
  function onResponse (err, res) {
    ...
    while (pending < MAX_CONCURRENCY && candidates.length) {
	  ...
	  //modif
	  var tmp = candidates.pop();
      var addr = tmp.addr
      query(addr,idToHexString(tmp.id))
    }
    ...
    if (pending === 0 && candidates.length === 0) {
      ...
      closest.forEach(function (contact) {
		...
		//console.log((opts?(opts.findNode?'find_node closest':'get_peers closest'):'get_peers closest')+' '+contact.addr + ' ' + idToHexString(contact.id)+' for '+idToHexString(id),true);
      })
  ...
  function queryClosest () {
    self.nodes.closest({ id: id }, K).forEach(function (contact) {
	  //modif
      query(contact.addr, idToHexString(contact.id))
    })
  }
...
DHT.prototype._onQuery = function (addr, message) {
  ...
  try {
	  ...
	  if (query === 'ping') {
		...
		//console.log('received query '+query+' from '+addr+' infohash '+idToHexString(message.a && message.a.info_hash));
		self._onGetPeers(addr, message)
	  } else if (query === 'announce_peer') {
		//console.log('received query '+query+' from '+addr+' infohash '+idToHexString(message.a && message.a.info_hash));//modifspies
		//modif
		var info = idToHexString(message.a && message.a.info_hash);
		if (info.length!==40) {
			console.log('strange announce');
		...
		//modif
		console.log('unexpected message from '+addr+' '+JSON.stringify(message)+' - query: '+query);
...
DHT.prototype._onResponseOrError = function (addr, type, message) {
  ...
  //modif
  if (message) {
	if (message.v&&message.r) {
		try {
			message.r.version=message.v.slice(0,2).toString('utf8')+message.v.slice(2,4).readUInt16BE();
		} catch(ee) {};
	};
  };
...
DHT.prototype._sendFindNode = function (addr, nodeId, cb) {
  ...
  function done (err, res) {
    ...
    if (res.nodes) {
      ...
      res.nodes.forEach(function (node) {
		//console.log('got node '+node.addr+' node id '+idToHexString(node.id)+' from '+addr+' for '+idToHexString(nodeId));
		self.emit('found_node',node.addr,idToHexString(node.id),addr);
  ...
  //console.log('send find node to '+addr+' for my nodeid '+self.nodeId.toString('hex')+' nodeId '+nodeId.toString('hex')+' distance '+KBucket.distance(self.nodeId.toString('hex'),nodeId.toString('hex')))
 ...
 */
//modif
DHT.prototype._sendGetPeers = function (addr, infoHash, cb, nid) {
  ...
  function done (err, res) {
    ...
    if (res.nodes) {
      res.nodes = parseNodeInfo(res.nodes)
      res.nodes.forEach(function (node) {
		//console.log('got node for get_peer '+node.addr+' node id '+idToHexString(node.id)+' from '+addr+' for infohash '+infoHash.toString('hex'));
        self.addNode(node.addr, node.id)
      })
    }
    ...
    if (res.values) {
      ...
      res.values.forEach(function (_addr) {
	    if (_addr.split(':')[0]!==self.myip) {
			//console.log('Found peer '+_addr+' received from '+addr+' nodeid '+nid+' for infohash '+idToHexString(infoHash),true);
			self._addPeer(_addr, infoHash, addr, idToHexString(res.id), res.version)
		} else {
...
DHT.prototype._onAnnouncePeer = function (addr, message) {
  ...
  //console.log('received announce peer from '+addr+' for '+infoHash+' port '+port+' implied port '+(message.a.implied_port!==0?addrData[1]:message.a.port)+' distance '+KBucket.distance(infoHash,self.nodeId.toString('hex')),true);
  ...
  self._send(addr, res)
  //modif
  self.emit('onAnnounce',addr,infoHash,port)
}
