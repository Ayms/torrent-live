...
DHT.prototype.addNode = function (addr, nodeId) {
  var self = this
  //modif
  if ((self._destroyed)||(addr.split(':')[0]===self.myip)) return
...
DHT.prototype._onData = function (data, rinfo) {
  ...
  //modif
  spies.call(self,'receiving message from know spy '+addr+' message type '+type+' '+(message.q?(message.q.toString()):(message.r?(message.r.toString()):'')),addr);
  ...
  function done (err, res) {
    ...
    if (res.values) {
      ...
      res.values.forEach(function (_addr) {
		//console.log('Peer '+_addr+' received from '+addr);
		//modif
		if (['67.215.246.10:6881','82.221.103.244:6881','91.121.60.42:6881'].indexOf(_addr)!==-1) {
